...
def createOperatorBundleAndIndexAndPushToDockerHub(operator_path: str, new_operator_tag: str, dockerhub_user: str, dockerhub_password: str, previous_tag: str = None) -> None:
    ...
    if previous_tag == None:
        call([opm_copy_path, "index", "add", "-c", "docker", "--bundles", "docker.io/snyk/kubernetes-operator-bundle" +
             ":" + new_operator_tag, "--tag", "docker.io/snyk/kubernetes-operator-index" + ":" + new_operator_tag])
    else:
        call([opm_copy_path, "index", "add", "-c", "docker", "--bundles", "docker.io/snyk/kubernetes-operator-bundle" + ":" + new_operator_tag, "--from-index",
             "docker.io/snyk/kubernetes-operator-index" + ":" + previous_tag, "--tag", "docker.io/snyk/kubernetes-operator-index" + ":" + new_operator_tag])
